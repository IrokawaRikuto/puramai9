// ±9(プラマイ・ナイン) Unity 人 vs CPU プロト(新ルール)
...
// 新ルール: コイン先出し / 前半2枚以上・後半1枚以上 / チャージ / HP非公開(ゲージ+色) /
//           ゾロ目ボーナス / エスカレーション / アイテム3種(①コイン反転 ②符号反転 ③ダブルシフト±7)。
...
public class PlusMinusNineGame : MonoBehaviour
{
    ...
    Button _confirmBtn, _itemBtn, _chargeBtn, _chargeConfirmBtn, _signBtn;
    ...
    bool _useItem; Phase _itemPhase; int _sign = 1; Card _charge; bool _chargeMode; Card _chargeCandidate;
    Coin _coin; bool _coinKnown;
    bool _confirmClicked;
    TurnPlan _myPlan;
    ItemType _myItem = ItemType.SignFlip;
    ...
    IEnumerator Run()
    {
        ...
        var cpuItem = (ItemType)Random.Range(1, 4);   // 1..3 = ①②③
    ...
    void ResetState()
    {
        _front.Clear(); _back.Clear(); _useItem = false; _charge = null; _chargeMode = false;
        _chargeCandidate = null; _coinKnown = false; _myPlan = null; _log.Clear();
        BuildPlayArea();
    }
    ...
    {
        _g.TurnHeadDraw();
        _front.Clear(); _back.Clear(); _useItem = false; _charge = null; _chargeMode = false; _sign = 1;
        _chargeCandidate = null;
        _myPlan = new TurnPlan();

        // --- ★コイン先出し(ラウンド頭に当たり偶奇を公開)---
        var coin = _g.TossCoin();
        _coin = coin; _coinKnown = true;
        var cpuPlan = _cpu.DecideFront(_g, Cpu, coin);    // CPUの前半を先に決定

        // --- 前半(あなた):2枚以上 ---
        _phase = UiPhase.Front;
        _coinText.text = coin == Coin.Odd ? "当たり偶奇:奇(ODD)" : "当たり偶奇:偶(EVEN)";
        _infoText.text = $"ターン{_g.Turn} 前半:カードを2〜3枚 → 確定";
        RefreshAll();
        do { yield return WaitConfirm(); if (_front.Count < 2) { AddLog("前半は2枚以上出します"); RefreshAll(); } }
        while (_front.Count < 2);
        _myPlan.Front = new List<Card>(_front);
        _myPlan.Charge = _charge;
        ...
        // --- 後半:1枚以上 ---
        _phase = UiPhase.Back;
        _infoText.text = $"ターン{_g.Turn} 後半:1〜2枚・コインを見て調整 → 確定";
        RefreshAll();
        do { yield return WaitConfirm(); if (_back.Count < 1) { AddLog("後半は1枚以上出します"); RefreshAll(); } }
        while (_back.Count < 1);
        ...
        if (_charge != null && _myPlan.Charge == null) _g.ApplyChargeNow(Me, _charge);  // 後半でチャージした場合
        ...
        AddLog($"T{_g.Turn} {(r.FinalCoin == Coin.Odd ? "奇" : "偶")}{esc}|あなた {r.FinalTotal[Me]}{Dir(r.Dir[Me])} / CPU {r.FinalTotal[Cpu]}{Dir(r.Dir[Cpu])}");
    ...
    void BuildPlayArea()
    {
        ...
        _oppHpText = MakeText(bg, 40, 35, 600, 30, "相手(CPU)", 22, TextAnchor.MiddleLeft);
        _oppFill = MakeBar(bg, 40, 70, 760, 26);
        _oppItemText = MakeText(bg, 820, 35, 400, 30, "相手アイテム", 18, TextAnchor.MiddleLeft);

        // 中央
        _coinText = MakeText(bg, 40, 160, 400, 40, "当たり偶奇:??", 26, TextAnchor.MiddleLeft);
        ...
        MakePanel(bg, 820, 285, 440, 230, new Color(0.10f, 0.13f, 0.20f));
        _logText = MakeText(bg, 835, 290, 415, 220, "", 15, TextAnchor.UpperLeft);

        // 自分パネル
        MakePanel(bg, 20, 525, 1240, 175, new Color(0.12f, 0.16f, 0.24f));
        _youHpText = MakeText(bg, 40, 535, 600, 30, "あなた", 22, TextAnchor.MiddleLeft);
        _youFill = MakeBar(bg, 40, 570, 760, 26);
        ...
        _chargeBtn = MakeButton(bg, 1040, 605, 130, 40, "チャージ", ToggleCharge, out _);
        _chargeConfirmBtn = MakeButton(bg, 1040, 650, 130, 40, "チャージ確定", ConfirmCharge, out _);
        _signBtn = MakeButton(bg, 1180, 605, 80, 40, "符号+", ToggleSign, out _signLabel);
    }

    IEnumerator PickItem()
    {
        var panel = MakePanel(_canvas.transform, 340, 220, 600, 260, new Color(0.12f, 0.16f, 0.24f));
        ...
        var opts = new (string, ItemType)[]
        {
            ...
            ("②符号反転(前半/後半・コスト3)", ItemType.SignFlip),
            ("③ダブルシフト±7(前半/後半・コスト2)", ItemType.DoubleShift),
        };
        bool picked = false;
        for (int i = 0; i < opts.Length; i++)
        {
            var it = opts[i].Item2;
            MakeButton(panel, 30, 60 + i * 56, 540, 48, opts[i].Item1, () => { _myItem = it; picked = true; }, out _);
        }
    ...
    void OnCardClicked(Card c)
    {
        if (_chargeMode)
        {
            if (_charge != null) return;                                  // 既にチャージ済み
            _chargeCandidate = (_chargeCandidate == c) ? null : c;        // 選ぶだけ。確定は「チャージ確定」
            RefreshAll(); return;
        }
        var sel = _phase == UiPhase.Front ? _front : _back;
        int max = _phase == UiPhase.Front ? 3 : 2;
        if (sel.Contains(c)) sel.Remove(c);
        else if (sel.Count < max && c != _charge) sel.Add(c);
        RefreshAll();
    }
    ...
    void ToggleItem()
    {
        ...
        var ph = _phase == UiPhase.Front ? Phase.Front : Phase.Back;
        if (!Items.CanUseInPhase(item, ph)) { AddLog("①コイン反転は後半のみ使用可"); return; }
        if (!_useItem)
        {
            if (_g.P[Me].Cost < Items.Cost(item)) { AddLog($"コスト不足(必要{Items.Cost(item)})"); return; }
        }
        _useItem = !_useItem;
        if (_useItem) _itemPhase = ph;
        RefreshAll();
    }

    void ToggleCharge()
    {
        if (_charge != null) { AddLog("このラウンドはチャージ済み(前後半で1枚のみ)"); RefreshAll(); return; }
        _chargeMode = !_chargeMode;
        if (!_chargeMode) _chargeCandidate = null;
        RefreshAll();
    }

    void ConfirmCharge()
    {
        if (!_chargeMode || _chargeCandidate == null) { AddLog("先にチャージするカードを選んでください"); return; }
        _charge = _chargeCandidate; _chargeCandidate = null; _chargeMode = false;
        AddLog($"チャージ:{_charge}");
        RefreshAll();
    }
    ...
    void RefreshAll()
    {
        ...
        _youCostText.text = $"チャージ:{me.Cost}";   // 自分の累計は自分には可視
        string chargeInfo = _charge != null ? "  チャージ:" + _charge
                        : _chargeMode ? (_chargeCandidate != null ? "  チャージ候補:" + _chargeCandidate + " →「チャージ確定」" : "  ◀ どれをチャージ?(カードを選択)")
                        : "";
        _stageText.text = "場(あなた) 前半:" + Join(_front) + "  後半:" + Join(_back) + chargeInfo;
    ...
    void UpdateDirBanner()
    {
        ...
        // ※ ①コイン反転の後出しがあるため、表示はあくまで「現時点の当たり偶奇」での予測
    ...
    void BuildHand()
    {
        ...
        {
            if (c == _charge) continue;                                   // チャージ確定済みは手札から消す
            if (_phase == UiPhase.Back && _front.Contains(c)) continue;   // 前半で出した札は手札から消す
            bool isCand = _chargeMode && c == _chargeCandidate;
            bool isSel = _chargeMode ? isCand : sel.Contains(c);
            var captured = c;
    ...
    int PreviewFinal()
    {
        ...
        bool useItem = frontItem || backItem;

        var tmp = new TurnPlan { Front = _front, Back = _back, UseItem = useItem, ShiftSign = _sign, GoldSign = _sign };
        tmp.ItemPhase = frontItem ? Phase.Front : Phase.Back;

        int count = _front.Count + _back.Count;
        bool gold = false;
        ...
        int total;
        if (gold) total = _sign * 9 * count + shiftF + shiftB;
        else if (GameState.FiveMatch(tmp, out int ms)) total = ms * 45 + shiftF + shiftB;
        else
        {
            int fr = GameState.ComputePhase(_front, 0, out bool fb);
    ...
    // HPゲージ:幅=HP割合 / 色=中央より上(＋寄り)=青・下(−寄り)=赤(数値は非公開)
    void UpdateBar(Image fill, int hp)
    {
        ...
        int mid = (_g.HpMin + _g.HpMax) / 2;
        fill.color = hp >= mid ? new Color(0.30f, 0.55f, 0.95f) : new Color(0.84f, 0.28f, 0.30f);
    }
    ...
    Image MakeBar(Transform parent, float x, float y, float w, float h)
    {
        var bgRt = MakePanel(parent, x, y, w, h, new Color(0.15f, 0.2f, 0.3f));
        // 中央(100/開始)ライン
        MakePanel(bgRt, w / 2f - 1, 0, 2, h, new Color(0.6f, 0.65f, 0.72f));
        var fgo = new GameObject("Fill", typeof(RectTransform), typeof(Image));
        fgo.transform.SetParent(bgRt, false);
        fgo.GetComponent<Image>().color = new Color(0.30f, 0.55f, 0.95f);
